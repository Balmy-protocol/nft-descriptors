import { DescriptorUtilsMock, DescriptorUtilsMock__factory } from '@typechained';
import { expect } from 'chai';
import { BigNumber, BigNumberish } from 'ethers';
import { ethers } from 'hardhat';

describe('DescriptorUtils', () => {
  let descriptorUtils: DescriptorUtilsMock;

  before('Setup accounts and contracts', async () => {
    const factory = (await ethers.getContractFactory('DescriptorUtilsMock')) as DescriptorUtilsMock__factory;
    descriptorUtils = await factory.deploy();
  });

  describe('#addressToString', () => {
    it('returns the correct string for a given address', async () => {
      let addressStr = await descriptorUtils.addressToString(`0x${'1234abcdef'.repeat(4)}`);
      expect(addressStr).to.eq('0x1234abcdef1234abcdef1234abcdef1234abcdef');
      addressStr = await descriptorUtils.addressToString(`0x${'1'.repeat(40)}`);
  ...
  describe('#fixedPointToDecimalString', () => {
    ...
    function calculateString(value: BigNumberish, base: BigNumberish, decimals: number) {
      ...
      return descriptorUtils.fixedPointToDecimalString(magnitude.mul(value).div(BigNumber.from(base)), decimals);
    }
  });
});
